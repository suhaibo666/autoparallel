...
> [!done] 2026-07-15 真机收口：116 已恢复，本节三项残留已用真机实测收口，见 **§6** 与
> [`realmachine/npu_closure_2026-07-15.md`](realmachine/npu_closure_2026-07-15.md)（5 个新真机点，预测 |误差| ≤ 0.66%）。
...
## 6. 真机收口（2026-07-15，116 恢复后）

§2/§3 的三项「待 116」残留已用真机实测收口。**5 个新 DSv3 缩层真机点**（116/shb.ms.2.9，FSDP-2、full 重算、
4 卡对并发采集），预测 vs 实测 |误差| ≤ **0.66%**。完整证据 [`realmachine/npu_closure_2026-07-15.md`](realmachine/npu_closure_2026-07-15.md)。

| cfg | 层 | seq | 实测 MiB | 预测 MiB | 预测/实测 |
|---|---|---|---|---|---|
| B | 2 | 4096 | 11733.0 | 11725.9 | 0.9994 |
| A | 4 | 4096 | 12473.1（锚点复现） | 12437.9 | 0.9972 |
...
- **P2-05（外推）CLOSED**：真机 peak 严格线性于层数（+740.1 MiB/2 层，四点零曲率）；评估器亦线性。外推误差**量化为 ≤ ~2.5%@61L**（评估器每层低估 ~14 MiB，略偏低、有界）。
- **P2-04（K_OPT/K_CE/kept_frag 泛化）CLOSED**：K_CE 的 CE∝seq 经 D 确认（seq 敏感度捕获 97.9%、D 绝对 0.49%）；K_OPT=4 层数无关且全配置 0.66% 内。**kept_frag=1.9 补 select_attn 4L 第 2 点（S：真机 14716.4 vs 预测 14812.6 → 1.0065），连原 8L 锚点（1.0024）跨层数泛化确认——不再单点拟合。**
- **P2-07（gate fp32）可达闭环**：离线字节测已闭；fp32 瞬态峰值建模族经 CE/D 真机背书；DSv4 专属聚合受测试 harness 仓库未注册 `deepseek_v4` 所限（跨仓库依赖，非本仓库缺陷）。

**最终**：§2 三项由「待 116」→ 真机全部收口（P2-04/P2-05 CLOSED，P2-07 可达闭环）；**NPU 定量残留归零**（kept_frag 已补第 2 点）。唯一范围外项 = P2-07 的 DSv4 专属聚合需跨仓库 harness。评估器口径经真机在 [2L,8L]×[2048,4096]×{full,select_attn} 全域确认 |误差| ≤ 0.66%。
