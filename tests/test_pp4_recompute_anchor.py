...
THEO_ON = {0: 16143.8, 1: 10924.6, 2: 11133.6, 3: 21008.9}
...
BAND_OFF = {0: (0.85, 0.94), 1: (0.83, 0.91), 2: (0.78, 0.86), 3: (0.79, 0.88)}
...
THEO_PP8 = {0: 20486.2, 1: 12879.2, 2: 11686.7, 3: 11477.1,
            4: 12495.2, 5: 11302.7, 6: 11093.1, 7: 25275.9}
...
_PP8_UNDER = {0, 3, 7}       # 仍 sim < real 的 stage（2026-07-30 起 **s3 加入**，见下方五次重钉）
...
#   未对任何 stage 调参。
_PP8_OVER_BAND = (1.00, 1.16)  # 翻成过读的 stage 的 ratio 带（钉住幅度，防再漂）
...
def test_pp8_framework_gap(peaks_pp8, stage):
    ...
    **举例（2026-07-30 移动，`docs/fused_mhc_branch_mismatch_2026-07-30.md` §5）**：
      - 欠读档 `_PP8_UNDER`：`{0,7}` → **`{0,3,7}`**。s3 由 1.073 落到 **0.963**——mHC 分支
        错配修掉后的真读数（上一轮已预告「错配一修就会退回边缘」，实测越过了边缘）。
      - 过读档：s1 1.045 / s2 **1.001** / s4 1.150 / s5 1.017 / s6 1.101（前次 1.09~1.27）。
        下界仍守 1.00：s2 现在只高出真机 8.7 MiB，是新的边缘位。"""
