...
# 2026-07-23 合账更新:①ON s1 经微批饱和 cap(mem_timeline _ctx_cap,185 pp8/P3-P 定标)
# 1.178→0.967 → 收紧到 ±5%;②OFF 各 stage 因 core_out 逆 RoPE 保留入账(真实成员,185 F 差分
# 背书)整体上移 [1.290,1.341,1.194,0.949]——过估根因仍为 mHC ×n 记账(残差②,未修),band 相应
# 放宽并保底 0.95/0.90(s3 欠侧收窄 0.922→0.949 ✓)。
BAND_ON = {0: (0.95, 1.05), 1: (0.95, 1.05), 2: (0.95, 1.05), 3: (0.95, 1.05)}
...
def test_recompute_net_saving_direction(peaks_on, peaks_off):
    ...
    # 2026-07-23:逆 RoPE 入账后 OFF 过估↑(mHC ×n 残差②未修)→ 仿真净省 15104 vs 真机 6242,
    # 松到 2.6×(仍能抓修前 23121 的 3.7× 病灶回归)。
    assert saving < 2.6 * real_saving, (
...
# 2026-07-23 微批饱和 cap 后收紧:中部 [1.62,1.49,1.30,1.32]→[1.23,1.23,1.04,1.18];
# s5 cap=2 vs 真机驻留 ~2.8 组 → 0.94(欠 6%,cap 律 ±1 组噪声,band 保底 0.92 注明)。
BAND_PP8 = {0: (0.95, 1.15), 1: (0.95, 1.28), 2: (0.95, 1.28), 3: (0.95, 1.10),
            4: (0.95, 1.23), 5: (0.92, 1.12), 6: (0.95, 1.05), 7: (0.90, 1.05)}
