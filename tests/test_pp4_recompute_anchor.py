...
# ⚠ **2026-07-30 起本锚点走「融合」mHC 分支**（`docs/fused_mhc_branch_mismatch_2026-07-30.md`）：
#   `_BASE` 新增 `"mhc_fused": "1"` —— 站点 yaml 逐字 `use_fused_mhc: true`
#   （`analysis/realmachine/ab_fusion_2026-07-25/dsv4h_{fused,unfused}_pp4_recomp.yaml:109`，
#   两份都写 true，差别只在 `apply_dsa_kernel_fusion`）；本文件 docstring 亦逐字「fused mHC」。
#   **恒等认亲**：本文件 `REAL_ON` 与 `tools/liveness_ab_validate.py:75` 的 run `a fused ON L8 m4`
#   逐位相同 → 八跑门与本门比的是**同一批真机跑**，而八跑门走 yaml 路早已是融合 mHC。
#   此前二者用两条不同的 mHC 分支给同一跑建模，本轮统一。
...
# 2026-07-29 四次重钉（kernel workspace 实测入账）：**只有 s2 动**，12161.9 → 12448.1
#   （+286.2）。加的是每个 r4 层 bwd 事件上真机实测的 730.0 MiB 融合稀疏 flash-MLA 反向
#   workspace（docs/kernel_workspace_2026-07-29.md，三点验证的实测律）；s0/s1/s3 的峰值
#   事件不在 r4 层的 bwd 上（s3 峰在 head/loss），故逐字节不动，s2 的峰值事件因此易主、
#   净上移 286.2 而非 730。
# 2026-07-30 五次重钉（**mHC 分支配置错配修复**，`docs/fused_mhc_branch_mismatch_2026-07-30.md`）：
#   s0 17458.2→**16143.8** / s1 12239.0→**10924.6** / s2 12448.1→**11133.6**（各 −1314.4）；
#   s3 21005.9→**21008.9**（+3.0）。机理：融合分支每模块 +32（−96 ln 吃 aggregated + 128 融合
#   ctx 的 x）vs 非融合 +480（−96 + 三份 fp32 副本 576），差 448/模块 = 896/层；全重算下解码层
#   只留 ci，该差只经 `remat_saves`/`recomp_scratch` 进峰，故净 −1314.4 而非 −896×层数。
#   s3 峰在 head/loss 段（不含解码层 saves），只吃融合 ctx 的净额 +3.0。
#   与真机比：s0 0.723→0.668 / s1 0.836→0.746 / s2 0.883→0.790 / s3 0.894→0.894 ——
#   四 stage 仍全部 `sim < real`（本门断言的不变量方向不动）。
# 2026-07-30 六次重钉（**逐层压缩比配置错配修复**，`docs/compress_ratios_mismatch_2026-07-30.md`）：
#   `_BASE` 新增 `"compress_ratios": "0,4,128,4,128,4,128,4"` —— 站点 yaml 逐字
#   （`analysis/realmachine/ab_fusion_2026-07-25/dsv4h_fused_pp4_recomp.yaml:121`，上一行注释
#   `# CSA (length must equal num_hidden_layers = 8)`）。此前扁平 query 没有 `compress_ratios`
#   旋钮 → 恒取预设 `dsv4_flash` 的 0/4/128 **循环近似** `(0,4,128,0,4,128,0,4)`，层型分布
#   3×r0/3×r4/2×r128，而站点表是 1×r0/4×r4/3×r128；三种层型驻留真机直测彼此不同
#   （r0 2235.1 / r4 2341.2 / r128 2116.1 MiB）→ 锚点一直在给**另一份层型分布**打分。
#   pp4 = 2 层/stage：s0=(L0,L1)=(r0,r4) **两表相同** → 逐 MiB 不变；s1=(L2,L3) r128,r0→r128,r4；
#   s2=(L4,L5) r4,r128→r128,r4；s3=(L6,L7) r0,r4→r128,r4。
#   s0 16143.8→**16143.8**（+0.0）/ s1 10924.6→**11973.1**（+1048.5）/
#   s2 11133.6→**11717.1**（+583.5）/ s3 21008.9→**21049.8**（+40.9，峰在 head/loss 段，
#   只吃到 L6 由 r0 换 r128 带进 `remat_saves` 的净额）。
#   与真机比：s0 0.668→0.668 / s1 0.746→**0.818** / s2 0.790→**0.831** / s3 0.894→**0.895**
#   —— 四 stage 仍全部 `sim < real`（不变量方向不动），缺口**收窄**。
# 2026-07-30 七次重钉（**r4 indexer 内部 RoPE 保留对入账**，`docs/r4_indexer_census_2026-07-30.md`）：
#   `CSAIndexer.forward_before_topk` 对 `q_pe` 那次 `ApplyRotaryPosEmb`（`indexer.py:182-187`）
#   走非融合分支（`apply_rope_fusion` 默认 False，`parallel_core/transformer_config.py:1578-1579`，
#   站点 yaml 未覆盖）→ `rope_utils.py:186-187` 两个 `mul` 各留一个操作数 → `t`/`t_rot` 各
#   `[S,B,64,64]` fp32 = **64.000 MiB**，**每 r4 层 +128.0 MiB**。r0/r128 无 indexer 对象
#   （`csa.py:608`）→ 逐字节不动。
#   pp4 = 2 层/stage，站点表下每 stage 恰好 1 个 r4 层：
#   s0 16143.8→**16243.8**（+100.0）/ s1 11973.1→**12101.1**（+128.0）/
#   s2 11717.1→**11845.1**（+128.0）/ s3 21049.8→**不变**（峰在 head/loss 段 `bwd@9`，
#   不含解码层 saves）。
#   ⚠ **s0 的 +100.0 不是「本项只算了一部分」**：该 stage 的**峰值事件由 `bwd@1`（r0 层）
#   换成了 `bwd@2`（r4 层）** —— 实测 before `bwd@2`=16115.8 < `bwd@1`=16143.8，after
#   `bwd@2`=16115.8+128.0=**16243.8** 越过之，故净差 100.0。r4 层那一格实涨仍是整 128.0。
#   与真机比：s0 0.668→**0.673** / s1 0.818→**0.827** / s2 0.831→**0.840** / s3 0.895→0.895
#   —— 四 stage 仍全部 `sim < real`（不变量方向不动），缺口继续**收窄**。
# 2026-07-30 重钉（`lm_head` 反向 kernel workspace 实测入账，docs/head_loss_bwd_workspace_2026-07-30.md）：
#   s3 21049.8 → **22143.8**（+1094.001 整 = 实测律在本站点的值本身，H=4096/S=4096/B=1）；
#   s0-s2 逐 MiB 不变（它们的峰值事件在解码层 bwd 上，本项挂在 head 段）。
#   比值 0.895 → **0.942**：仍 OOM-不安全，缺口收窄。
THEO_ON = {0: 16243.8, 1: 12101.1, 2: 11845.1, 3: 22143.8}
...
# 2026-07-30 四次重钉（**mHC 分支配置错配修复**，`docs/fused_mhc_branch_mismatch_2026-07-30.md`）：
#   ratio = **0.892 / 0.867 / 0.815 / 0.832**（前次 1.239 / 1.243 / 1.111 / 0.926）。
#   无重算 = 全量 saves，故直接吃满两条分支的 896 MiB/层 之差（s0 −10538.7 / s1 −7903.1 /
#   s2 −5267.8 / s3 −2632.5，差异来自各 stage 峰值事件在世的层数/在途深度不同）。
#   上一轮的 what-if 预测 0.868/0.867/0.815/0.832 —— s1/s2/s3 **逐位命中**；s0 预测 0.868、
#   实测 0.892，差额来自 what-if 之后入账的 kernel workspace（+730 实测项，
#   `docs/kernel_workspace_2026-07-29.md`），不是本轮调参。
#   s0–s2 由过读**翻回欠读**（= OOM-不安全侧），s3 一直欠读且更欠 —— **如实记带、不调参掩盖**；
#   方向上这批 0.82–0.89 与 167 逐层直测（0.89–0.99×）同侧同量级，而修前的 1.11–1.24 与之矛盾。
# 2026-07-30 五次重钉（**逐层压缩比配置错配修复**，`docs/compress_ratios_mismatch_2026-07-30.md`）：
#   ratio = **0.892 / 0.922 / 0.859 / 0.833**（前次 0.892 / 0.867 / 0.815 / 0.832）。
#   无重算 = 全量 saves，故层型换档直接进峰：s0 两表相同 → **逐 MiB 不变**（27112.7）；
#   s1 +1158.0（L3 r0→r4）/ s2 +786.8（L4,L5 r4,r128→r128,r4）/ s3 +32.7（峰在 head/loss 段）。
#   上一轮的 what-if 预测（`docs/fused_mhc_branch_mismatch_2026-07-30.md` §9）s1 0.922 / s2 0.859
#   —— **逐位命中**。四点仍全部欠读（OOM-不安全），但 s1/s2 更靠近 167 逐层直测的 0.89–0.99×。
#   **只移动 s1/s2 两条带**（各按新读数平移、带宽不变 ±0.04）；s0/s3 读数在原带内，带不动。
# 2026-07-30 六次重钉（**r4 indexer 内部 RoPE 保留对入账**，见 `THEO_ON` 上方说明）：
#   ratio = **0.909 / 0.940 / 0.874 / 0.838**（前次 0.892 / 0.922 / 0.859 / 0.833）。
#   无重算 = 全量 saves 且逐微批常驻，故幅度 = **在途微批深度 × 128.0**：
#   s0 +512.0（深度 4）/ s1 +384.0（3）/ s2 +256.0（2）/ s3 +128.0（1）
#   —— 27112.7→27624.7 / 19383.6→19767.6 / 15297.4→15553.4 / 23176.7→23304.7。
#   四点仍全部欠读（OOM-不安全），缺口继续收窄，且更靠近 167 逐层直测的 0.89–0.99×。
#   **四条带一条没动**：新读数全部落在原带内（0.909∈(0.85,0.94)、0.940∈(0.88,0.96)、
#   0.874∈(0.82,0.90)、0.838∈(0.79,0.88)）—— 按既有规矩，读数不出带就不平移带。
BAND_OFF = {0: (0.85, 0.94), 1: (0.88, 0.96), 2: (0.82, 0.90), 3: (0.79, 0.88)}
...
_BASE = {
    ...
    # `mhc_fused`（2026-07-30 新增）：站点 yaml `use_fused_mhc: true`
    #   （`analysis/realmachine/ab_fusion_2026-07-25/dsv4h_fused_pp4_recomp.yaml:109`）。
    "hc": "4", "mhc_fused": "1", "dsa_fused": "1", "ce_fused": "1",
    # `compress_ratios`（2026-07-30 新增，`docs/compress_ratios_mismatch_2026-07-30.md`）：
    #   站点 yaml 逐字 `compress_ratios: [0, 4, 128, 4, 128, 4, 128, 4]`
    #   （同上文件 `:121`；上一行注释 `# CSA (length must equal num_hidden_layers = 8)`）。
    #   此前无此旋钮 → 恒取预设 `dsv4_flash` 的**循环近似** `(0,4,128,0,4,128,0,4)`
    #   （预设的 `source` 字符串自己就写着「循环近似」）→ 层型分布 3×r0/3×r4/2×r128，
    #   而真机是 1×r0/4×r4/3×r128。旁证：167/2026-07-29 逐层直测的每层型数值正是按
    #   「r4 = **4 层**均值 / r128 = **3 层**均值」算的（`docs/census_fix_residual_carrier_2026-07-29.md` §2）
    #   —— 测量本身带着站点的层型分布。
...
# 2026-07-29 四次重钉（kernel workspace 实测入账）：s2 同 THEO_ON（+286.2）；
#   s3 28875.0 → **29605.0**（+730.0 整 —— MTP 尾 stage 的峰值事件正落在 r4 层的 bwd 上）。
# 2026-07-30 五次重钉（mHC 分支配置错配修复）：s0-s2 同 `THEO_ON`（各 −1314.4）；
...
# 2026-07-30 七次重钉（**逐层压缩比配置错配修复**，`docs/compress_ratios_mismatch_2026-07-30.md`）：
#   s0-s2 同 `THEO_ON`（+0.0 / +1048.5 / +583.5）；s3 29789.8 → **29830.6**（+40.8）。
#   MTP 尾 stage 的峰值事件落在 MTP decoder 层的 bwd 上，而 MTP 层的压缩比取 `ratios[-1]`
#   （`cost_eval/layers/head.py:250-252`）—— 两张表的末项**都是 4**，故 MTP 层自身不换档；
#   这 +40.8 与无 MTP 的 s3 同源（该 stage 的 L6 由 r0 换成 r128）。
#   四 stage 仍全部 sim < real（s3 0.747→0.748）。
# 2026-07-30 八次重钉（**r4 indexer 内部 RoPE 保留对入账**，见 `THEO_ON` 上方说明）：
#   s0-s2 同 `THEO_ON`（+100.0 / +128.0 / +128.0，s0 的 +100.0 是峰值事件易主，见那里）；
#   s3 29830.6 → **29958.6**（+128.0）。MTP 尾 stage 的峰值事件落在 **MTP decoder 层的 bwd**
#   上，而 MTP 层的压缩比取 `ratios[-1]`（`cost_eval/layers/head.py:250-252`）= **4** → 它
#   自己就是一个 r4 层、自带 indexer → 吃满一份 128.0。
#   四 stage 仍全部 sim < real（s3 0.748→**0.751**）。
THEO_MTP = {0: 16243.8, 1: 12101.1, 2: 11845.1, 3: 29958.6}
...
# 2026-07-29 四次重钉（kernel workspace 实测入账）：**只有 s1/s4 动**，各 +730.0 整。
#   pp8·L8 = 1 层/stage，`compress_ratios=[0,4,128,4,128,4,128,4]` → 只有 s1/s3/s5/s7 是
#   r4 层；其中 s1/s4 的峰值事件落在该层的 bwd 上（s4 是 s3 的层？见 stage 映射），其余
#   stage 的峰在 gather/remat 事件上、未被 730 顶过 → 逐字节不动。
# 2026-07-30 五次重钉（**mHC 分支配置错配修复**）：s0-s6 各 **−1314.4**（1 层/stage，与 pp4
#   的解码层 stage 同幅）；s7 25272.9→**25275.9**（+3.0，峰在 head/loss 段）。
#   s0 21800.6→20486.2 / s1 14193.7→12879.2 / s2 13001.1→11686.7 / s3 12791.6→11477.1 /
#   s4 13809.7→12495.2 / s5 12617.1→11302.7 / s6 12407.6→11093.1。
# 2026-07-30 六次重钉（**逐层压缩比配置错配修复**，`docs/compress_ratios_mismatch_2026-07-30.md`）：
#   pp8·L8 = **1 层/stage**，故 stage s 的层型 = `ratios[s]`，逐 stage 一一对应：
#     预设循环 (0,4,128,0,4,128,0,4) → s0 r0 / s1 r4 / s2 r128 / s3 r0 / s4 r4 / s5 r128 / s6 r0 / s7 r4
#     站点表   (0,4,128,4,128,4,128,4) → s0 r0 / s1 r4 / s2 r128 / s3 **r4** / s4 **r128** / s5 **r4** / s6 **r128** / s7 r4
#   → 只有 s3/s4/s5/s6 换档：s3 +1146.1（r0→r4，含该层 bwd 上真机实测的 730.0 融合稀疏
#   flash-MLA workspace）/ s4 −1064.5（r4→r128，退掉那 730.0 与 r4 的索引器 saves）/
#   s5 +1064.5（r128→r4）/ s6 +81.6（r0→r128）。s0/s1/s2/s7 两表同档 → **逐 MiB 不变**。
#   s3 11477.1→**12623.2** / s4 12495.2→**11430.7** / s5 11302.7→**12367.2** / s6 11093.1→**11174.7**。
#   ⚠ 顺带订正一条注解：上面「四次重钉」写的「`compress_ratios=[0,4,128,4,128,4,128,4]` →
#   只有 s1/s3/s5/s7 是 r4 层」当时**说的是站点表、而模型用的是预设循环**（那句话是错配本身的
#   影子）。本轮之后模型确实用站点表，该句才成立。
# 2026-07-30 七次重钉（**r4 indexer 内部 RoPE 保留对入账**，见 `THEO_ON` 上方说明）：
#   pp8·L8 = **1 层/stage**，站点表 `[0,4,128,4,128,4,128,4]` → 只有 s1/s3/s5/s7 是 r4 层；
#   其中 s7 的峰在 head/loss 段（`bwd@9`，不含解码层 saves）→ **只有 s1/s3/s5 动，各 +128.0 整**。
#   s0/s2/s4/s6 是 r0/r128 层（无 `CSAIndexer` 对象，`csa.py:608`）→ **逐字节不动**。
#   s1 12879.2→**13007.2** / s3 12623.2→**12751.2** / s5 12367.2→**12495.2**。
THEO_PP8 = {0: 20486.2, 1: 13007.2, 2: 11686.7, 3: 12751.2,
            # 2026-07-30：s7 25275.9 → **26369.9**（+1094.001 整，同上）；s0-s6 逐 MiB
            #   不变（各 stage 1 层，只有 s7 带 head 段）。比值 0.956 → **0.997**。
            4: 11430.7, 5: 12495.2, 6: 11174.7, 7: 26369.9}
...
_PP8_UNDER = {0, 7}          # 仍 sim < real 的 stage（2026-07-30 六次重钉：s3 又离开，见下方）
...
# 2026-07-29 四次重钉：1.00–1.25 → **1.00–1.29**。实测 s1 1.152 / s2 1.113 / s3 1.073 /
#   s4 **1.271** / s5 1.135 / s6 1.232。上移只来自 s1/s4 各加一笔 730.0 MiB 的**实测** bwd
#   kernel workspace —— 该量是真机测出来的，不是调参；它把已经过读的 stage 推得更过读，
#   如实记账。下界仍刻意保持 1.00（真翻成欠读必须变红）。
# 2026-07-30 五次重钉（**mHC 分支配置错配修复**，`docs/fused_mhc_branch_mismatch_2026-07-30.md`）：
#   1.00–1.29 → **1.00–1.16**。实测 s1 1.045 / s2 **1.001** / s4 1.150 / s5 1.017 / s6 1.101。
#   ⚠ **上一轮预告的「s3 一修错配就退回边缘」发生了，而且越过了边缘**：s3 由 1.073 落到
#   **0.963**（sim 11477.1 vs 真机 11919.0，**低 441.9 MiB**）→ 已**翻成欠读**。
#   处理方式是把 s3 **移进 `_PP8_UNDER`**（改断言为 `sim < real`，与 s0/s7 同一条不变量），
#   **不是**把本带下界调到 0.96 把红压回绿 —— 下界 1.00 正是上一轮为此刻设的哨兵。
#   **新的边缘位是 s2 = 1.001**（sim 11686.7 vs 11678.0，高 8.7 MiB；上一轮 s3 是高 8.6 MiB）。
#   未对任何 stage 调参。
# 2026-07-30 六次重钉（**逐层压缩比配置错配修复**，`docs/compress_ratios_mismatch_2026-07-30.md`）：
#   1.00–1.16 → **1.00–1.12**。实测 s1 1.045 / s2 **1.001** / s3 **1.059** / s4 1.052 /
#   s5 1.113 / s6 1.109。
#   ⚠ **s3 又翻回过读**：上一轮它刚从 1.073 落到 0.963（被移进 `_PP8_UNDER`），本轮站点表把
#   它由 r0 层换成 r4 层（+1146.1）→ **1.059**，故**移出** `_PP8_UNDER`。上一轮的 what-if
#   预测 s3 → 1.059，**逐位命中**。两次移动都是**如实跟随读数**，不是为了守住某个方向。
#   ⚠ **边缘位 s2 = 1.001 逐 MiB 不动**（sim 11686.7 vs 真机 11678.0，仍高 8.7 MiB）：
#   pp8 每 stage 1 层，s2 的层型在两张表里**都是 r128** → 本轮修复对它恒等。**没有越界。**
#   过读上界由 1.150(s4) 变成 1.113(s5) → 带上界随读数收到 1.12（留 ~0.007 余量，与上一轮
#   1.150→1.16 的留法同款）；下界仍刻意守 1.00（真翻成欠读必须变红）。
# 2026-07-30 七次重钉（**r4 indexer 内部 RoPE 保留对入账**，见 `THEO_PP8` 上方说明）：
#   1.00–1.12 → **1.00–1.13**。实测 s1 **1.055** / s2 **1.001** / s3 **1.070** / s4 1.052 /
#   s5 **1.124** / s6 1.109。只有 r4 的三个 stage（s1/s3/s5）上移，各 +128.0；r0/r128 的
#   s2/s4/s6 逐 MiB 不动。
#   ⚠ **边缘位 s2 = 1.001 又一次逐 MiB 不动**（sim 11686.7 vs 真机 11678.0，仍高 8.7 MiB）：
#   s2 是 **r128 层**，没有 `CSAIndexer` 对象 → 本轮对它**恒等**。**没有越界。**
#   过读上界由 1.113(s5) 变成 **1.124(s5)** → 带上界随读数放到 1.13（留 ~0.006 余量，与上两轮
#   1.150→1.16 / 1.113→1.12 的留法同款）；下界仍刻意守 1.00（真翻成欠读必须变红）。
#   `_PP8_UNDER` 仍是 {0,7}（s0 0.827 / s7 0.956，两者本轮都不动）—— **未移动任何 stage 的档**。
_PP8_OVER_BAND = (1.00, 1.13)  # 翻成过读的 stage 的 ratio 带（钉住幅度，防再漂）
...
def test_pp8_framework_gap(peaks_pp8, stage):
    ...
    **不变量（未变）**：每个 stage 的方向被钉死——要么 `sim < real`（欠读，OOM-不安全，
    与 s0/s7 同源的框架释放缺口），要么落在已记录的过读带内；两边都不许悄悄漂或被"调回去"。

    **举例（2026-07-30 二次移动，`docs/compress_ratios_mismatch_2026-07-30.md` §5）**：
      - 欠读档 `_PP8_UNDER`：`{0,3,7}` → **`{0,7}`**。s3 由 0.963 升到 **1.059**——逐层压缩比
        错配修掉后它由 r0 层变成 r4 层（+1146.1）。上一轮的 what-if 预测 1.059，逐位命中。
      - 过读档：s1 1.045 / s2 **1.001** / s3 1.059 / s4 1.052 / s5 1.113 / s6 1.109。
        下界仍守 1.00：s2 仍只高出真机 8.7 MiB，是边缘位，且本轮对它**恒等**（两表都是 r128）。

    （前次移动，`docs/fused_mhc_branch_mismatch_2026-07-30.md` §5：`{0,7}` → `{0,3,7}`，
    s3 由 1.073 落到 0.963。两次都是如实跟随读数，没有为守方向调过参。）"""
