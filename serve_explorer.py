...
def graph_json(layers, norm_dtype, spec, dims, recompute=None):
    ...
    for l in layers:
        ...
        #   - full  ：`checkpoint_input`（**仅层入口边界**，2026-07-24 纯理论口径——经验 ctx 免疫已删，
        #     全重算 save_for_backward 全部释放；真机每微批层 ~1.9G 驻留是框架释放缺口，见 FrameworkGap
        #     警示，不进层卡数字）。
        ...
        for i, (op, oop) in enumerate(zip(l.ops, orig_ops)):
            ...
            for t, ot in zip(op.saves, oop.saves):
                ...
                # 被重算的 op 前向不存任何 saves（反向重物化）。2026-07-24 纯理论口径：全重算下
                # save_for_backward 全部释放，无 ctx 免疫例外。
...
function drawGraph(st){
  ...
  document.getElementById("gpane").innerHTML=st.graph.map((L,idx)=>{
    ...
    const amH=rcOnL
      ? `<span class="am" style="color:#8e44ad" title="重算态(纯理论口径,2026-07-24):仅存下方标注为「存」的激活;被重算 op 的 saves 前向不存(反向重物化)。全重算只留层入口 checkpoint_input(层 construct 入参 bf16);save_for_backward 全部释放(报告§7.9)。真机每微批层 ~1.9G 驻留是框架释放缺口,见页面 FrameworkGap 警示,不进此数字。">↻${L.recomp==="full"?"全重算":"选择性重算"} · 存 ${L.act_mib} MiB${L.entry_mib>0?"（层入口锚点 "+L.entry_mib+"M）":""} / 全量 ${L.full_act_mib}</span>`
