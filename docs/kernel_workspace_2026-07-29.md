...
| ③ | **前向 304.001 MiB**（融合 mHC pre-sinkhorn kernel，每层 2 次） | 每层 max 304.001 | 已测、已归属（输出签名逐张匹配，S=4096/2048 两点确认 **S 无关**），但它的 op 在 `cost_eval/layers/residual.py` —— **本轮不属本人可改范围**。模型当前 fwd `workspace` 只有 16.0 MiB（`_fa_workspace`），欠读 288 MiB/层。**交接项**。<br>**→ 2026-07-30 已闭合，见 [`mhc_fwd_workspace_2026-07-30.md`](mhc_fwd_workspace_2026-07-30.md)。**已挂在 `_fused_hc_ops` 的两个 `*_hc_pre_sinkhorn` op 上（每层 2 次，与本文 §4.3 对上），三个层型的层内 fwd `workspace` 现均为 **304.001 MiB**，与本文 §4.2 的窗口 `max_ws` 逐位吻合。⚠ **锚点一个都没动**——量到事件线的 36 个锚点/stage 峰值事件**全部是 BWD 事件**，前向侧的项结构上顶不到（融合-mHC 里头寸最小的 `pp4 OFF s0` 还差 4015.1 MiB）。本行「S=4096/2048 两点」与该文 §9 三次跑（1024/2048/4096）的点数记载不一致，已在新文 §1.2 如实记下；因本项建成**常数**，两种读法结果等价。 |
