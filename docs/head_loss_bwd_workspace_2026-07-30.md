...
| ③ | **既有 `bwd_scratch` 的 `K_CE`** | 实测 loss 层反向**共存 3 张瞬态满 vocab fp32 平面**；`K_CE−1` 在 pp>1 非 lean 下取 7 → 多 4 份 = 8080 MiB。**本轮一个字节没动**（单列为发现，不折进本项）。要动它须重钉一批 DSv3-era 冻结口径的锚点，属另一条任务线。<br>**→ 2026-07-30 已闭合，见 [`k_ce_recalibration_2026-07-30.md`](k_ce_recalibration_2026-07-30.md)。**⚠ 该轮**订正了本行的归因**：本行说的「多 4 份 = 8080 MiB」**不成立** —— 本轮量到 3 张的那个 167 站点，模型侧 `cross_entropy_fused=True` → `loss_lids` 空（`mem_timeline.py:324`）→ **`K_CE` 那一行根本不执行**（该站点实测 `bwd_scratch` = 2.00 张平面，不是 7 张）。把两侧放到同一条路径上、用 9 份仓内 profiler CSV 逐块重数后：真正的过读是**恰好 1 张**（= 代码注释自己写的「+1 保守」），`K_CE` 由 `4/8` 改为 **`3/7`**（`lean` 无逐块台账、仍 4）。位移只有 3 条锚点，OOM-不安全 23 → 26。 |
...
### 8.2 ⚠ 15 个锚点由欠读**翻成过读**（1.02–1.08）—— 成因已定位并量化

> **精确计数**（`join` 逐行比对 before/after，48 条锚点/stage）：**25 条上移、23 条逐位不动、
> 15 条由 <1.0 翻到 ≥1.0**；OOM-不安全总数 **38 → 23**。（`088eb19` 的 commit message 里
> 写的「14 个」是手数时漏了 `116 std gqa pp1 s0`，以本节为准。）
...
> **⚠ 2026-07-30 订正上面这一段**（[`k_ce_recalibration_2026-07-30.md`](k_ce_recalibration_2026-07-30.md) §1）：
> 上表右列把 167 站点的实测数与 `K_CE=8` 并列比较，**但该站点走不到 `K_CE` 那一行**
> —— 它 `cross_entropy_fused=True` ⇒ `loss_lids` 空（`mem_timeline.py:324`）⇒ 模型在那里记的是
> **2.00 张**平面（`8·S·B·vocab` 原值）。故「多记 4 张 = 8080 MiB」**在任何锚点上都没发生过**；
> 该站点反而是**欠读 1 张**。真正走 `K_CE=8` 的只有 `pp2-stage1` 一条锚点，用它自己那份
> profiler CSV 逐块数出来是 **5 张 fp32 + 5 张 bf16 = 7.5 fp32-等效** ⇒ `K_CE` 应为 **7**（多记 1 张）。
> `K_CE` 已按此改为 `3/7`（`lean` 仍 4）。

**按任务书要求，`bwd_scratch` 的标定问题单列为发现、本轮一个字节没动**（§7 ③）。
因此这 15 个锚点现在停在过读侧 = **OOM-安全侧**；band 只上移到刚好封住实测落点（两侧都守），
**没有为了凑回 ~1.00 而削本项**。
...
1. ~~**`bwd_scratch` 的 `K_CE` 重标定**（§7 ③）~~ —— **2026-07-30 已做，见
   [`k_ce_recalibration_2026-07-30.md`](k_ce_recalibration_2026-07-30.md)。**
   ⚠ 本条的两个预判都被那一轮证伪：① 不是「多记 4 张」而是**多记 1 张**（本行依据的
   167 站点走 fused-CE 分支、`K_CE` 不执行）；② 修它**没有**把 15 条新过读拉回 ~1.0 —— 只拉动
   **3 条**（其余 12 条不在 `K_CE` 路径上），且那 3 条是**跌回欠读**、OOM-不安全 23 → 26。
